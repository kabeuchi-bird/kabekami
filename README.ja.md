...
[`crates/kabekami-common/i18n/ja.toml`](crates/kabekami-common/i18n/ja.toml) をコピーし、
言語コードのファイル名（例: `fr.toml`）で下記のいずれかに置いてください。

| 置き場所 | 適用範囲 |
|---|---|
| `$KABEKAMI_I18N_DIR` | 明示的な上書き（テスト用に便利） |
| `~/.config/kabekami/i18n/` | 自分のユーザーのみ |
| `/usr/share/kabekami/i18n/` | 全ユーザー |

追加した言語は `kabekami-config` の言語ドロップダウンに自動で並びます。

未記載のキーは英語にフォールバックするので、**部分的な翻訳でも問題なく動作します**。
訳せるところだけ訳しておけば、残りは英語のまま読める状態が保たれます。英語と日本語は
バイナリに埋め込まれているため、ファイルを 1 つも設置しなくても利用できます。
