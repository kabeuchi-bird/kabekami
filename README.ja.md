...
- **再起動しても状態を維持**: 一時停止・現在の壁紙・表示モード・切り替え間隔
...
一時停止の状態と現在の壁紙は再起動をまたいで維持されます。ログアウト前に一時停止して
おけば次回ログイン時も停止したままで、「ゴミ箱に移動」などの操作も最初の切り替えを待たず
に画面上の壁紙へ効きます。**表示モード**と**切り替え間隔**をトレイから変更した場合は
`config.toml` に保存されます。
...
> **KRunner で実行する場合の注意**: KRunner（Alt+Space）で `kabekami --next` 等を実行すると、ShellRunner と AppRunner / HistoryRunner の両方が同じ入力にマッチして二重発火することがあります。kabekami 側で 500 ms のスロットルを入れているため通常の二重発火は吸収されますが、安定して使いたい場合は下記の **グローバルショートカット** の利用を推奨します。
...
このほか `~/.config/kabekami/state.toml` に、ユーザー設定ではなく実行時の状態（一時停止
の有無・現在の壁紙）を保存します。自動で管理されるため編集は不要です。読み込みは起動時の
1 回だけなので、状態をリセットしたい場合はデーモンを停止してからファイルを削除し、再度
起動してください（動作中に削除しても反映されず、次の保存で書き直されます）。ブラック
リストは `~/.config/kabekami/blacklist.txt` に別途保存されます。

### 表示言語の追加

UI 文字列は言語ごとの TOML ファイルに分かれているため、言語の追加に**再ビルドは不要**です。
雛形として `ja.toml` をコピーしてください（[ソースツリー](crates/kabekami-common/i18n/ja.toml)、
またはパッケージから導入した場合は `/usr/share/kabekami/i18n/ja.toml`）。言語コードの
ファイル名（例: `fr.toml`）にして、下記のいずれかに置きます。

| 置き場所 | 適用範囲 | 優先度 |
|---|---|---|
| `/usr/share/kabekami/i18n/` | 全ユーザー | 最低 |
| `~/.config/kabekami/i18n/` | 自分のユーザーのみ | ↓ |
| `$KABEKAMI_I18N_DIR` | 明示的な上書き（テスト用に便利） | 最高 |

追加した言語は `kabekami-config` の言語ドロップダウンに自動で並びます。
言語ファイルの読み込みは起動時の 1 回だけなので、追加・編集したら
**kabekami と設定 GUI を再起動してください**。

**ファイルは置き換えではなく重ね合わせです。** 訳語を 1 つだけ変えたい場合、2 行の
ファイルで足ります。書かなかったキーは優先度の低い側の翻訳がそのまま使われるため、
上書きしたまま残りの翻訳の更新にも追従できます。

```toml
# ~/.config/kabekami/i18n/ja.toml — 1 つの訳語だけ差し替える
[tray]
quit = "おわり"
```

どこにも書かれていないキーは英語にフォールバックするので、**部分的な翻訳でも問題なく
動作します**。英語と日本語はバイナリに埋め込まれているため、ファイルを 1 つも設置しなくても
利用できます。
