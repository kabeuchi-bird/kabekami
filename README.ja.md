...
**壁紙がぼやける／ネイティブ解像度で表示されない** — kabekami はまず `kscreen-doctor --json`（Plasma 6 で安定）を試し、失敗時は従来のテキスト出力を解析します。両方失敗した場合は 1920×1080 にフォールバックし、その後 60 秒ごとに再検出を試みます（起動時に KScreen デーモンが間に合わなかった場合でも、後から自動で正しい解像度に切り替わります）。原因切り分けには `RUST_LOG=kabekami=debug kabekami` を実行して `kscreen-doctor output` 行を確認してください。即時の回避策は `KABEKAMI_SCREEN=2560x1440`（実際の解像度）の指定です。
